...
function MacOSSafari({ url = 'https://example.com', tabs = null, theme = 'light', scale = 1, width = 860, height = 580, children }) {
        ...
        <div style={{ background: chromeBg, borderBottom: `1px solid ${borderColor}` }}>
          <div style={{ display: 'flex', alignItems: 'center', height: 38, paddingLeft: 12 }}>
            <div style={{ display: 'flex', gap: 7, marginRight: 16, flexShrink: 0 }}>
              {[['#ff5f57','#e0443e'],['#febc2e','#d4a017'],['#28c840','#1aab29']].map(([color, hover], i) => (
                <div key={i} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 0, flex: 1, overflowX: 'auto', height: '100%' }}>
              {defaultTabs.map((tab, i) => (
                <div key={i} onClick={() => setActiveTab(i)} style={{
                  ...
                  {tab.title}
                  <span style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', fontSize: 10, color: subColor, opacity: .7 }}>✕</span>
                </div>
              ))}
              <div style={{ padding: '0 10px', cursor: 'pointer', color: subColor, fontSize: 18, lineHeight: '30px', opacity: .7 }}>+</div>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', gap: 0 }}>
              {['‹','›'].map((arr, i) => (
                <button key={i} style={{
                  ...
                  color: i === 0 ? textColor : subColor, fontSize: 20, padding: '0 6px',
                  opacity: i === 0 ? 1 : .4,
                }}>{arr}</button>
              ))}
            </div>
            <div style={{
              flex: 1, height: 28, background: urlBarBg,
              borderRadius: 8, border: `1px solid ${borderColor}`,
              display: 'flex', alignItems: 'center', gap: 6, padding: '0 10px',
            }}>
              <svg width="11" height="13" viewBox="0 0 11 13" fill="none">
                <path d="M5.5 1C3 1 1 3 1 5.5S3 10 5.5 10 10 8 10 5.5 8 1 5.5 1Z" stroke={subColor} strokeWidth="1.2"/>
              ...
              <input
                ...
                style={{
                  ...
                  fontSize: 12, color: textColor, fontFamily: 'inherit',
                }}
              />
            </div>
            <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: subColor, fontSize: 15, padding: '0 4px' }}>⬆</button>
          </div>
        </div>
