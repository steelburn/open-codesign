...
function CalendarComponent() {
  ...
  const isToday = (d, thisMonth) => thisMonth && d === today.getDate() && month === today.getMonth() && year === today.getFullYear();
  const isSelected = (d, thisMonth) => thisMonth && selectedDate === dateStr(d);

  return (
    <div style={{ fontFamily: 'Helvetica Neue, sans-serif', background: '#fff', borderRadius: 14, border: '1px solid #e4e4e7', overflow: 'hidden', position: 'relative', userSelect: 'none' }}>
      <div style={{ padding: '16px 20px', borderBottom: '1px solid #f4f4f5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#18181b', letterSpacing: -.3 }}>{monthLabel}</h2>
          <button onClick={() => { setCurrent(new Date(2026, 3, 1)); setSelectedDate(null); }}
            style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #e4e4e7', background: '#fafafa', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', color: '#52525b' }}>
            Today
          </button>
        </div>
        <div style={{ display: 'flex', gap: 4 }}>
          {['‹','›'].map((arr, i) => (
            <button key={arr} onClick={() => setCurrent(new Date(year, month + (i === 0 ? -1 : 1), 1))}
              style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid #e4e4e7', background: '#fff', cursor: 'pointer', fontSize: 16, color: '#52525b', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>
              {arr}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', borderBottom: '1px solid #f4f4f5' }}>
        {weekdays.map(d => (
          <div key={d} style={{ padding: '8px 0', textAlign: 'center', fontSize: 11, fontWeight: 600, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: .5 }}>{d}</div>
        ))}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', position: 'relative' }} data-cal>
        {cells.map((cell, idx) => {
          ...
          return (
            <div key={idx}
              ...
              style={{
                ...
                background: sel ? TWEAK_DEFAULTS.selectedBg : isWeekend && cell.thisMonth ? '#fafafa' : '#fff',
              ...
              <div style={{
                ...
                color: today_ ? '#fff' : cell.thisMonth ? (sel ? TWEAK_DEFAULTS.accent : '#18181b') : '#d4d4d8',
                marginBottom: 4,
              }}>
                {cell.day}
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {evts.slice(0, 2).map(evt => (
                  <div key={evt.id}
                    onMouseEnter={e => {
                      ...
                      setTooltip({ event: evt, x: rect.left - parentRect.left, y: rect.bottom - parentRect.top + 6 });
                    }}
                    ...
                    style={{
                      ...
                      fontSize: 10, fontWeight: 500, color: evt.color,
                ...
                {evts.length > 2 && (
                  <div style={{ fontSize: 10, color: '#a1a1aa', paddingLeft: 4 }}>+{evts.length - 2} more</div>
                )}
              </div>
            </div>
          );
        ...
        {tooltip && (
          <div style={{
            position: 'absolute', left: Math.min(tooltip.x, 540), top: tooltip.y,
            background: '#18181b', color: '#fff', borderRadius: 10, padding: '10px 14px',
            fontSize: 12, boxShadow: '0 8px 24px rgba(0,0,0,.18)',
            pointerEvents: 'none', minWidth: 160, zIndex: 30,
          }}>
            <div style={{ fontWeight: 700, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: tooltip.event.color, display: 'inline-block', flexShrink: 0 }} />
              {tooltip.event.title}
            </div>
            <div style={{ color: '#a1a1aa', fontSize: 11 }}>
              {new Date(tooltip.event.date + 'T00:00').toLocaleDateString('en', { weekday: 'short', month: 'short', day: 'numeric' })}
              {' · '}{tooltip.event.time}
            </div>
          </div>
        )}
      ...
      {selectedDate && (() => {
        ...
        return (
          <div style={{ borderTop: '1px solid #f4f4f5', padding: '16px 20px', background: '#fafafa' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#52525b', marginBottom: 10, textTransform: 'uppercase', letterSpacing: .5 }}>
              {new Date(selectedDate + 'T00:00').toLocaleDateString('en', { weekday: 'long', month: 'long', day: 'numeric' })}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {dayEvts.map(evt => (
                <div key={evt.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px', background: '#fff', borderRadius: 8, border: '1px solid #f4f4f5' }}>
                  <div style={{ width: 4, height: 36, borderRadius: 2, background: evt.color, flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#18181b' }}>{evt.title}</div>
